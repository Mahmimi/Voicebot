...
df = pd.read_excel('temp.xlsx', usecols=['path','spaced_text', 'duration'], engine='openpyxl')
df['path'] = '/mnt/nas/asr/tts/datasets/syn/' + df['path']
...
def tokenize_and_convert(example, tokenizer_instance, error_indices):
    original_text = example["spaced_text"]
    ...
    try:
        ...
        print(f"Error processing example at index {example['__index_level_0__']}: {original_text}")
        print(f"Error details: {e}")

        # Add the index to the list of error indices
        error_indices.append(example['__index_level_0__'])
...
# # Print intermediate steps for one example
# example_index = 439
# original_text = ds_train["spaced_text"][example_index]
# input_ids = tokenizer_instance._tokenize(original_text)
# label_numbers = [tokenizer_instance._convert_token_to_id(token) for token in input_ids]
...
# Display the updated "input_ids" for the first example in the training dataset

org_text = ds_train["spaced_text"][0]
print('org_text', org_text)

input_ids = ds_train["input_ids"][0]
print('input_ids', input_ids)


# decoded_text = tokenizer.decode(input_ids, skip_special_tokens=True)
# decoded_text = [tokenizer_instance._convert_id_to_token(label_number) for label_number in token_lst]
...
name = 'speecht5_syn_data9k_step5k'
